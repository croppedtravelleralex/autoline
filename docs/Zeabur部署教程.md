...
部署完成后，请按照以下步骤验证前后端连接是否正常。

### 1. 验证后端服务

直接在浏览器中访问后端的健康检查接口：

- 地址：`https://<你的后端域名>/api/health`
- 预期返回：
  ```json
  {"status": "ok", "service": "AutoLine Monitor"}
  ```

如果无法访问或返回 502/504 错误，请检查：
- Zeabur 控制台的后端服务日志（Logs）
- 确保端口号（PORT）配置正确（默认 8001）

### 2. 验证前端连接

打开前端页面，按下 `F12` 打开浏览器开发者工具，切换到 **Network (网络)** 标签页。

1. 刷新页面。
2. 观察以 `/api/` 开头的请求（如 `settings` 或 `state`）。
3. **正常情况**：状态码为 `200 OK`。
4. **异常情况**：
    - **404 Not Found**：请求路径错误，检查 `VITE_API_URL` 是否包含多余的 `/` 或后缀。
    - **502 Bad Gateway**：后端服务未启动或挂掉。
    - **CORS Error**（跨域错误）：检查后端 `main.py` 中的 `CORSMiddleware` 配置，确保允许前端域名访问。
    - **Mixed Content**：如果前端是 HTTPS，后端必须也是 HTTPS（Zeabur 默认提供 HTTPS，无需担心）。
